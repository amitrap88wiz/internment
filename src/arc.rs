...
impl<T: Eq + Hash + Send + Sync + 'static> ArcIntern<T> {
    ...
    pub fn new(val: T) -> ArcIntern<T> {
        Self::get_or_try_insert(val, None::<fn(&T) -> Option<EvictCallback<T>>>).unwrap()
    }

    /// Intern a value, optionally with a callback for new insertions.
    ///
    /// Returns `Some(intern)` if the value was interned (existing or new),
    /// or `None` if the factory rejected the insertion.
    ///
    /// # Arguments
    /// * `should_insert` - Optional factory called only for new values:
    ///   - `None`: Always insert without an eviction callback
    ///   - `Some(factory)`: Call `factory(&val)` which returns:
    ///     - `Some(evict_cb)`: Insert and register `evict_cb` for when value is evicted
    ///     - `None`: Reject insertion, return `None` from this method
    ///
    /// The factory pattern defers resource allocation (e.g., quota reservation) until
    /// we know the value is actually new, avoiding overhead for existing values.
    pub fn get_or_try_insert<F, G>(mut val: T, mut should_insert: Option<F>) -> Option<ArcIntern<T>>
    where
        F: FnMut(&T) -> Option<G>,
        G: FnOnce(&T) + Send + Sync + 'static,
    {
        loop {
            ...
            if let Some(b) = m.get_mut(&val) {
                ...
                match m.entry(BoxRefCount(b)) {
                    Entry::Vacant(e) => {
                        let on_evict: Option<EvictCallback<T>> = match &mut should_insert {
                            Some(should_insert) => Some(Box::new(should_insert(&e.key().0.data)?)),
                            None => None,
                        };
                        let p = ArcIntern {
...
fn test_get_or_try_insert() {
    ...
    const TEST_STR: &str = "test_get_or_try_insert_unique";
...

    {
        ...
        let cb_factory_count = factory_call_count.clone();

        let x = ArcIntern::get_or_try_insert(
            TEST_STR.to_string(),
            Some(|s: &String| {
                assert_eq!(s, TEST_STR);
                cb_factory_count.fetch_add(1, Ordering::SeqCst);
                let evict_count = cb_evict_count.clone();
                Some(move |s: &String| {
                    assert_eq!(s, TEST_STR);
        ...
        // Factory should have been called once for new insertion
        assert_eq!(factory_call_count.load(Ordering::SeqCst), 1);

        // Same value interned again should NOT call the factory
        let factory_count_before = factory_call_count.load(Ordering::SeqCst);
        let y = ArcIntern::get_or_try_insert(
            TEST_STR.to_string(),
            Some(|_: &String| {
        ...
        // Evict callback should not have been called yet
        assert_eq!(evict_count.load(Ordering::SeqCst), 0);
    }

    // After all references dropped, evict callback should have been called once
